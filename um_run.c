 ...
 * Returns: nothing
*/
void free_prog(um_seq seg_mem, uint32_t* registers)
{
    assert(registers);
    assert(seg_mem);
             ...
             int* counter)
{
    ...
    switch (instruction[0]) {
        ...
        case 6: nand(instruction, registers);
            break;
        case 7: halt(instruction, registers, seg_mem);
            break;
...
{
    um_seq seg_mem = (um_seq)initialize(seg_zero, seg_zero_size);

    int counter = 0;

    uint32_t * registers = calloc(1, sizeof(uint32_t)*8);
    assert(registers);
    uint32_t * instruction = calloc(4, sizeof(uint32_t));
    assert(instruction);

    while(counter < get_seg_length(seg_mem, 0)) {
        instruction = parse(segment_get_val(seg_mem, 0, counter), instruction);
        counter++;
